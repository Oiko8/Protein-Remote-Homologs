...
def IVFPQ(exe_path, data_path, query_path, k_neighbors=10,
          kclusters=64, nprobe=4, M=16, nbits=8):
    """
    Run IVFPQ executable and return neighbors[q] = list of indices.
    Keeps the same stdout parsing.
    """
    ...
    if result.returncode != 0:
        ...
        print("STDOUT:\n", result.stdout)
        print("STDERR:\n", result.stderr)
        raise RuntimeError("IVFPQ failed")
...
def run_ivfpq(data_file="protein_vectors.dat", query_file="protein_queries.dat", knn=5):
    ...
    neighbors, tApprox, qps = IVFPQ(exe_path=exe_path, query_path=query_path, data_path=data_path,
