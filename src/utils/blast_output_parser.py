...
def build_sblast_sets(blast_by_query, n_ground_truth, max_evalue=1e-3):
    ...
    for q, hits in blast_by_query.items():
        ...
        best_by_sseqid: Dict[str, BlastHit] = {}
        for h in filtered:
            prev = best_by_sseqid.get(h.sseqid)
            if prev is None or h.bitscore > prev.bitscore:
                best_by_sseqid[h.sseqid] = h

        uniq = list(best_by_sseqid.values())
        uniq.sort(key=lambda h: h.bitscore, reverse=True)

        top = uniq[:n_ground_truth] if n_ground_truth > 0 else []
        out[q] = {h.sseqid for h in top}
    return out


def build_sblast_sets_identity_band(blast_by_query, n_ground_truth,
                                    min_pident=None, max_pident=None, max_evalue=1e-3):
    out: Dict[str, Set[str]] = {}
    for q, hits in blast_by_query.items():
        filtered = [h for h in hits if h.evalue <= max_evalue]
...
def recall_at_n(ann_ranked_sseqids, blast_top_set, n):
    ...
    inter = ann_top & blast_top_set
    # print("n=", n, " |ann_top=", len(ann_top), " |blast_set=", len(blast_top_set), " |inter=", len(inter))
    # print("ANN Top")
    # print(ann_top)
    # print("---------------------------------------------------------------------------------")
    # print("Blast top")
    # print(blast_top_set)
    # print("==================================================================================")
    return len(inter) / len(blast_top_set)
...
def blast_ann_comparison(blast_tsv, ann_results, db_ids_path, query_ids_path, n_ground_truth=5, ann_n=5,
                         ...
                         remote_max_pident=30.0, close_min_pident=30.0):
    ...
    # Adding recall for remote and close homologs
